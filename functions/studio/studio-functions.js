...
function reverseCharacters(stringName){
    if(typeof(stringName) === "string"){
    return stringName.split('').reverse().join('');
} else
{
    return Number(String(stringName).split('').reverse().join(''));
}
}
...
function reverseArray(array){
    ...
    for (let index = 0; index < array.length; index++) {
        reversedArray.push(reverseCharacters(array[index]));
    }
...
function funPhrase(str){
    let modifiedStr = "";
    if(str.length <= 3){
        modifiedStr = str.slice(str.length-1, str.length);
    }else{
